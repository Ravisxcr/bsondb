"""Tests for BsonDBClient: a local facade over a data directory, not a
...
import bsondb
from bsondb.database import Database


def test_construction_creates_data_directory(tmp_path):
    data_dir = tmp_path / "data"
    assert not data_dir.exists()
    client = bsondb.BsonDBClient(str(data_dir))
...
def test_attribute_access_returns_database(tmp_path):
    client = bsondb.BsonDBClient(str(tmp_path / "data"))
