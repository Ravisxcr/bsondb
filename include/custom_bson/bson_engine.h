...
/* Zero-copy: *out points into r->buf, valid as long as the underlying
 * buffer is. */
 ...
 * the terminating NUL. */
bson_status_t bson_reader_read_cstring_bounded(bson_reader_t *r, size_t bound,
...
const char *bson_iter_key(const bson_iter_t *it, size_t *len); /* zero-copy */
...
bson_status_t bson_iter_value_binary(const bson_iter_t *it, uint8_t *subtype,
...
bson_status_t bson_writer_append_element_header(bson_writer_t *w, uint8_t type,
