...
## `custom_bson.encode(document: dict) -> bytes`

Serializes a Python `dict` to BSON bytes. Raises:

- `TypeError` -- `document` is not a `dict`.
- `custom_bson.InvalidDocument` -- a key is not `str` (or contains an
...
- `custom_bson.DocumentTooLarge` -- the encoded document would exceed 16 MiB.
- `MemoryError` -- allocation failure.

## `custom_bson.decode(data: bytes) -> dict`
...
- `custom_bson.InvalidBSON` -- the bytes are truncated, malformed, or
...
- `custom_bson.BSONNotImplementedError` -- the document contains a
...
## `custom_bson.ObjectId`

A 12-byte unique identifier, matching MongoDB/BSON's ObjectId format.
Pure Python (`python/custom_bson/object_id.py`) -- see the module
...
| `None` | Null | |
| `custom_bson.ObjectId` | ObjectId | |
| `datetime.datetime` | UTC datetime | naive datetimes are treated as already-UTC; decode always returns naive datetimes |

## `custom_bson.MongoClient` / `Database` / `Collection`

```python
client = custom_bson.MongoClient("./data")   # local data directory, not a network connection; creates it if missing
db = client.mydb                              # -> Database (a subdirectory of client.path)
coll = db.mycollection                        # -> Collection (one <name>.cbd mmap'd data file)
```

`MongoClient(path)` creates `path` eagerly (there's no server to defer
...
(`custom_bson.decode()` + `python/custom_bson/query.py`'s `matches()`),
